...
mod validator_args {
    ...
    async fn paginator_query_handler(
        payload: summer_web::validation::validator::ValidatorEx<
            summer_web::axum::extract::Query<Paginator>,
        >,
    ) -> Json<serde_json::Value> {
        Json(serde_json::json!({
            "page_size": payload.page_size,
    ...
    async fn paginator_path_handler(
        payload: summer_web::validation::validator::ValidatorEx<
            summer_web::axum::extract::Path<Paginator>,
        >,
    ) -> Json<serde_json::Value> {
        Json(serde_json::json!({
            "page_size": payload.page_size,
    ...
    async fn paginator_form_handler(
        payload: summer_web::validation::validator::ValidatorEx<
            summer_web::axum::extract::Form<Paginator>,
        >,
    ) -> Json<serde_json::Value> {
        Json(serde_json::json!({
            "page_size": payload.page_size,
    ...
    async fn router_with_rules(rules: Option<PageRules>) -> Router {
        ...
        let router = Router::new()
            ...
            .route(
                "/paginator-query",
                axum::routing::get(paginator_query_handler),
            )
            .route(
                "/paginator-path/{page_size}",
                axum::routing::get(paginator_path_handler),
            )
            .route(
                "/paginator-form",
                axum::routing::post(paginator_form_handler),
            );
