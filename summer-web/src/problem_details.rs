...
impl ProblemDetails {
    ...
    pub fn internal_server_error() -> Self {
        Self::new(500).with_detail("An unexpected error occurred while processing your request")
    }
...
mod tests {
    ...
    fn test_validation_error_uses_dedicated_type() {
        let p = ProblemDetails::validation_error(vec![Violation::body("name", "required")]);
        assert_eq!(p.status, 400);
    ...
    fn test_builder_chain() {
        ...
        assert_eq!(p.extensions.get("retryAfter"), Some(&serde_json::json!(30)));
    }
    ...
    fn test_serialize_with_violations() {
        let p = ProblemDetails::validation_error(vec![Violation::body("email", "invalid")]);
        let json = serde_json::to_value(&p).unwrap();
