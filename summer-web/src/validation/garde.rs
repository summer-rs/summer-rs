//! Garde integration for RFC 9457 ProblemDetails.
...
/// Garde wrapper for extractors implementing `garde::Validate`.
    ...
    T::Context: Send + Sync + 'static,
{
    let result = if TypeId::of::<T::Context>() == TypeId::of::<()>() {
        ...
        let context = context.ok_or(GardeInternalError::MissingContext(std::any::type_name::<
            T::Context,
        >()))?;
...
where
    ...
    T: Validate,
    T::Context: Clone + Send + Sync + 'static,
    axum::extract::Form<T>: FromRequest<State, Rejection = axum::extract::rejection::FormRejection>,
{
    type Rejection = ProblemDetails;
